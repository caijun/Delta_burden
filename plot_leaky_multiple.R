...
files <- list.files("output/multiple", full.names = T)
for (file in files) {
  # file <- files[6]
  ...
  outfile <- glue("figs/multiple/epi_curve_R0-{R0str}_ve2-{ve2str}_init-f-{init_fstr}_Tonset-{Tonset}_strategy-{strategy}_nsim-{nsim}.tif")
  ...
  outfile <- glue("figs/multiple/2dose_coverage_R0-{R0str}_ve2-{ve2str}_init-f-{init_fstr}_Tonset-{Tonset}_strategy-{strategy}_nsim-{nsim}.tif")
  tiff(outfile, width = 12, height = 10, unit = "in", res = 300, compression = "lzw")
  print(p3)
  dev.off()
