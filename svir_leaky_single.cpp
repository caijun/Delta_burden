...
double max_eigenval(vector<vector<double>> mat, int n)
{
    ...
    //     double eval_i_real = GSL_REAL(eval_i);
    //     double eval_i_imag = GSL_IMAG(eval_i);
    //     if (eval_i_imag >= 0)
    //     {
    //         printf("eigenvalue = %g+%gi\n", eval_i_real, eval_i_imag);
    //     }
    //     else
    //     {
    //         printf("eigenvalue = %g%gi\n", eval_i_real, eval_i_imag);
...
int main(int argc, char *argv[])
{
    ...
    // cout << doses.size() << endl;
    // cout << doses.back() << endl;
    // cout << doses[doses.size() - 1] << endl;

    // sprintf(path, "data/cm/contact_matrix%d", 1);
    // cm = readMatrix(path, ngr, ngr, cm);
    // max_eval = max_eigenval(cm, ngr);
    // cout << max_eval << endl;
