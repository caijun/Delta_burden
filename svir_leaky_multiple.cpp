...
int main(int argc, char *argv[])
{
    ...
    int newI[ngr][nvar];                                            //counter of daily number of new infections by age
        ...
        for (int t = 0; t < (Tonset + Tmax) * zeta; t++)
        {
            ...
            for (int i = 0; i < ngr; i++)
            {
                ...
                for (int k = 0; k < nvar; k++)
                {
                    ...
                    double prob_foi_ve1 = 1.0 - exp(-(1.0 - ve1[k]) * foi / zeta);
                    double prob_foi_ve2 = 1.0 - exp(-(1.0 - ve2[k]) * foi / zeta);
                    ...
                    for (int j = 0; j < delay2 * zeta; j++)
                    {
                        nV1_Imat[i][j][k] = gsl_ran_binomial(rng, prob_foi_ve1, V1mat[i][j]);
                        nV1_I[i][k] += nV1_Imat[i][j][k];
                    }
                    //among vaccinated V2
                    nV2_I[i][k] = gsl_ran_binomial(rng, prob_foi_ve2, V2[i]);
